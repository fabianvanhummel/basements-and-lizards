...
export const GameTab = ({ book, gameState, handleAction }) => {
  const [reactions, setReactions] = useState(gameState.reactions);

  useEffect(() => setReactions(gameState.reactions), [gameState])
  ...
  const componentMap = {
    ...
    INSPECT_ITEM: () => <div>Item</div>,
    APPROACH_NPC: () => <div>NPC</div>,
  };
  const ComponentToRender = componentMap[gameState.action.type];
...
const Reaction = ({ type, dismiss, ...props }) => (
